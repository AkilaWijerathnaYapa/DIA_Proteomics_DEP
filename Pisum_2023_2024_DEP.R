...
jpeg("DEP/Protein_counts_BarPlot.jpg", height = 15*350, width = 25*350, res = 800)
p
dev.off()
...
jpeg("DEP/heatmap_Pisum_common_proteins.jpg", width = 100*350,  height = 42*350,res=1000)
out
dev.off()
...
jpeg("DEP/BOXPLOT_Pisum_Comparison.jpg", height = 15*350, width = 40*350,res=500)
...
jpeg("DEP/PCA_Pisum_DEP.jpg", height = 30*350, width = 35*350,res=1200)
p
dev.off()
...
jpeg("DEP/heatmap_Pisum_DEP_top50.jpg", width = 90*350,  height = 42*350,res=1000)
out
dev.off()


#exporting heatmap matrix
write.csv(top50_pro, "DEP/top50_heatmap_data.csv",row.names=T)

#exporting intensity data of top50 proteins for each replicate
pro_intensity = aggregate(value ~ Var1+Var2, data = sig_pro_m, FUN = mean)		# aggregating for proteins
pro_intensity_w = pro_intensity %>% pivot_wider(names_from = Var2, values_from = value)%>% as.data.frame()

write.csv(pro_intensity_w, "DEP/pro_intensity_data.csv",row.names=F)
...
jpeg("DEP/heatmapOfcontrast_Pisum_DEP_top50.jpg", width = 90*350, height = 42*350,res=1000)
out
dev.off()
...
jpeg("DEP/Volcano_Pisum_DEP.jpg", height = 45*350, width = 55*350, res = 1200)
p
dev.off()	
...
write.csv(data_results, 'DEP/Pisum_DEP_result_table.csv',row.names=F)
...
write.csv(Apical_Buds_2h_vs_Apical_Buds_0h_result, "DEP/Apical_Buds_2h_vs_Apical_Buds_0h_DEP_result_table.csv",row.names=F)
...
write.csv(Axillary_Buds_0h_vs_Apical_Buds_0h_result, "DEP/Axillary_Buds_0h_vs_Apical_Buds_0h_DEP_result_table.csv",row.names=F)
...
write.csv(Axillary_Buds_2h_vs_Axillary_Buds_0h_result, "DEP/Axillary_Buds_2h_vs_Axillary_Buds_0h_DEP_result_table.csv",row.names=F)
...
jpeg("DEP/Pisum_venn.jpg",height=25*350,width=35*350,res=1200)
ggvenn(
  x, 
...
jpeg("DEP/Pisum_DEP_vennDiagram.jpg", height = 20*350, width = 30*350, res = 800)
venn
dev.off()
...
#UpsetR plot
jpeg("DEP/upsetPlot.jpg",height = 10*350, width = 20*350, res = 300)
...
write.csv(common,"DEP/Pisum_COMMON_DEP.csv")
...
write.xlsx(uncommon,"DEP/Pisum_UNCOMMON_DEP.xlsx")
